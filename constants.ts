...
export const TRANSLATIONS: Record<string, Record<Language, string>> = {
  ...
  apiKeyInstructionsTitle: { en: 'Action Required: Configure API Key', ar: 'إجراء مطلوب: تهيئة مفتاح API' },
  apiKeyInstructionsP1: { en: 'This application requires a Google AI API Key to function. It seems it has not been configured correctly in the deployment environment.', ar: 'يتطلب هذا التطبيق مفتاح Google AI API ليعمل. يبدو أنه لم يتم إعداده بشكل صحيح في بيئة النشر.' },
  apiKeyInstructionsP2_github: { en: 'If you are deploying via GitHub Actions, please follow these two steps:', ar: 'إذا كنت تقوم بالنشر عبر GitHub Actions، يرجى اتباع هاتين الخطوتين:' },
  apiKeyInstructionsStep1: { en: '1. Create a Repository Secret:', ar: '١. إنشاء سر للمستودع (Repository Secret):' },
  apiKeyInstructionsStep1_detail: { en: 'Go to your GitHub repository\'s `Settings` > `Secrets and variables` > `Actions`. Create a new secret named exactly `API_KEY` with your key as the value.', ar: 'اذهب إلى `Settings` في مستودعك على GitHub، ثم `Secrets and variables` > `Actions`. أنشئ سراً جديداً باسم `API_KEY` بالضبط، وضع مفتاحك كقيمة له.' },
  apiKeyInstructionsStep2: { en: '2. Update Your Workflow File:', ar: '٢. تحديث ملف سير العمل (Workflow):' },
  apiKeyInstructionsStep2_detail: { en: 'In your workflow file (e.g., `.github/workflows/deploy.yml`), find the build step and add the `env` variable to pass the secret. It should look like this:', ar: 'في ملف سير العمل الخاص بك (مثلاً: `.github/workflows/deploy.yml`)، ابحث عن خطوة البناء (build step) وأضف متغير `env` لتمرير السر. يجب أن يبدو هكذا:' },
};
